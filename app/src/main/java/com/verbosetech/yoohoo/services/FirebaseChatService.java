...
public class FirebaseChatService extends Service {
    ...
    private ChildEventListener chatUpdateListener = new ChildEventListener() {
        ...
        public void onChildRemoved(DataSnapshot dataSnapshot) {
            ...
            if (chat != null) {
                ...
                if (realmList.size() == 0) RealmObject.deleteFromRealm(chat);
                else {
    ...
    public void onCreate() {
        ...
        Notification notification = new NotificationCompat.Builder(this, CHANNEL_ID_MAIN).setSmallIcon(R.drawable.noti_icon).setContentTitle(getString(R.string.app_name)).setContentText("Chat service running").setSound(null).build();
    ...
    private void uploadAndSend(final File fileToUpload, final Attachment attachment, final int attachmentType, final String chatChild, final String recipientId, final User recipientUser, final Group recipientGroup) {
    ...
    private void sendMessage(String messageBody, @AttachmentTypes.AttachmentType int attachmentType, Attachment attachment, String chatChild, String userOrGroupId, User recipientUser, Group recipientGroup) {
        //Create message object
        Message message = new Message();
        message.setAttachmentType(attachmentType);
        if (attachmentType != AttachmentTypes.NONE_TEXT) message.setAttachment(attachment);
    ...
    private void notifyNewGroup(Group group) {
        ...
        notificationBuilder.setSmallIcon(R.drawable.noti_icon).setContentTitle("Group: " + group.getName()).setContentText("You have been added to new group called " + group.getName()).setAutoCancel(true).setSound(defaultSoundUri).setContentIntent(pendingIntent);
        ...
        usersRef.child(userId).addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                try {
                    User user = dataSnapshot.getValue(User.class);
                    if (User.validate(user)) {
                        if (updateUserInDb(user)) broadcastUser("changed", user);
                    }
                } catch (Exception ex) {
                    Log.e("USER", "invalid user");
                }
    ...
    private void saveMessage(Message message) {
        ...
        if (!isMeSender) chat.setRead(false);
        ...
        if (!message.isDelivered() && !message.getSenderId().equals(myId) && !helper.isUserMute(message.getSenderId()) && (Helper.CURRENT_CHAT_ID == null || !Helper.CURRENT_CHAT_ID.equals(userOrGroupId))) {
            ...
            Uri defaultSoundUri = RingtoneManager.getDefaultUri(RingtoneManager.TYPE_NOTIFICATION);
            notificationBuilder.setSmallIcon(R.drawable.noti_icon).setContentTitle(chat.getChatName()).setContentText(message.getBody()).setAutoCancel(true).setSound(defaultSoundUri).setContentIntent(pendingIntent);
            int msgId = 0;
            try {
