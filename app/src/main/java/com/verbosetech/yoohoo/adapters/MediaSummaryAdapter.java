                ...
                public void onClick(View v) {
                    if (file != null && file.exists()) {
                        if (dataList.get(getAdapterPosition()).getAttachmentType() == AttachmentTypes.IMAGE) {
                            context.startActivity(ImageViewerActivity.newFileInstance(context, Environment.getExternalStorageDirectory() + "/" + context.getString(R.string.app_name) + "/" + AttachmentTypes.getTypeName(dataList.get(getAdapterPosition()).getAttachmentType()) + (myId.equals(dataList.get(getAdapterPosition()).getSenderId()) ? "/.sent/" : ""), dataList.get(getAdapterPosition()).getAttachment().getName()));
                        } else {
                            Intent intent = new Intent(Intent.ACTION_VIEW);
                            Uri uri = MyFileProvider.getUriForFile(context, context.getString(R.string.authority), file);
                    ...
                    else Toast.makeText(context, R.string.file_not_found, Toast.LENGTH_LONG).show();
                }
            });
        }

        public void setData(Message msg) {
            file = new File(Environment.getExternalStorageDirectory() + "/" + context.getString(R.string.app_name) + "/" + AttachmentTypes.getTypeName(msg.getAttachmentType()) + (myId.equals(msg.getSenderId()) ? "/.sent/" : ""), msg.getAttachment().getName());
