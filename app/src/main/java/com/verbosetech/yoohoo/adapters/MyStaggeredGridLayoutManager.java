...
public class MyStaggeredGridLayoutManager extends StaggeredGridLayoutManager {
    ...
    public void onMeasure(RecyclerView.Recycler recycler, RecyclerView.State state, int widthSpec, int heightSpec) {
        ...
        for (int i = 0; i < getItemCount(); i++) {
            ...
            if (!measureScrapChild(recycler, i, View.MeasureSpec.makeMeasureSpec(i, View.MeasureSpec.UNSPECIFIED), View.MeasureSpec.makeMeasureSpec(i, View.MeasureSpec.UNSPECIFIED), mMeasuredDimension))
                continue;
    ...
    private boolean measureScrapChild(RecyclerView.Recycler recycler, int position, int widthSpec, int heightSpec, Point measuredDimension) {
        ...
        if (view != null) {
            ...
            measuredDimension.set(view.getMeasuredWidth() + p.leftMargin + p.rightMargin, view.getMeasuredHeight() + p.bottomMargin + p.topMargin);
