...
public class ChatDetailFragment extends Fragment implements GroupNewParticipantsAdapter.ParticipantClickListener {
    ...
    private void showAddMemberConfirmationDialog() {
        ConfirmationDialogFragment confirmationDialogFragment = ConfirmationDialogFragment.newInstance(getString(R.string.add_member_title), String.format(getString(R.string.add_member_message), (groupNewUsers.size() == 1 ? groupNewUsers.get(0).getNameToDisplay() : String.format(getString(R.string.member_count), groupNewUsers.size()))), new View.OnClickListener() {
            @Override
            public void onClick(View view) {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        if (mListener != null) mListener.getAttachments();
        if (user != null) {
            setData();
            ...
            mediaSummaryViewAll.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if (mListener != null) mListener.switchToMediaFragment();
                }
            });
    ...
    private void setParticipants() {
        ...
        for (MyString memberId : group.getUserIds()) {
            ...
            if (pos != -1) groupUsers.add(myUsers.get(pos));
        }
        ...
        switch (requestCode) {
            case CALL_REQUEST_CODE:
                if (grantResults[0] == PackageManager.PERMISSION_GRANTED)
                    callPhone(true, user.getId());
                else callPhone(false, user.getId());
                break;
        }
    }
    ...
    public void onParticipantClick(final int pos, final User participant) {
        ConfirmationDialogFragment confirmationDialogFragment = ConfirmationDialogFragment.newInstance(getString(R.string.remove_user_title), String.format(getString(R.string.remove_user_message), participant.getNameToDisplay()), new View.OnClickListener() {
            @Override
            public void onClick(View view) {
    ...
    private void removeParticipant(String id) {
        ...
        for (MyString userId : group.getUserIds())
            if (!userId.getString().equals(id)) userIds.add(userId);
